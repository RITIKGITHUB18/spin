...
export function ErrorState({
  ...
  return (
    <div className="flex min-h-svh flex-col items-center justify-center gap-4 bg-cream-50 px-7.5 py-12 text-center animate-fade-up">
      ...
      <div>
        <div className="font-serif text-2xl text-cream-900">{title}</div>
      ...
      {actionLabel && onAction && (
        <button
          type="button"
          onClick={onAction}
          className="mt-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-500 px-5.5 py-3.5 text-[14.5px] font-bold text-white"
        >
          {actionLabel}
        </button>
      )}
